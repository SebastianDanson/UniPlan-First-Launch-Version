...
class AddTaskViewController: PickerViewController {
    ...
    func setupViews() {
        ...
        //Not topView
        titleHeading.anchor(top: topView.bottomAnchor, left: view.leftAnchor, paddingTop: 15, paddingLeft: 20)
        titleTextField.centerX(in: view)
        titleTextField.anchor(top: titleHeading.bottomAnchor, paddingTop: 2)
        
        startDateHeading.anchor(top: titleTextField.bottomAnchor, left: view.leftAnchor, paddingTop: 15, paddingLeft: 20)
        startDatePicker.centerX(in: view)
        startDatePicker.anchor(top: startDateHeading.bottomAnchor)
        
        durationHeading.anchor(top: startDatePicker.bottomAnchor, left: view.leftAnchor, paddingTop: 10, paddingLeft: 20)
        ...
        saveButton.anchor(top: reminderButton.bottomAnchor, paddingTop: 20)
        ...
        if let taskIndex = TaskService.shared.getTaskIndex() {
            if let task = TaskService.shared.getTask(atIndex: taskIndex) {
                ...
                reminderSwitch.isOn = task.reminder
                reminderButton.isHidden = !task.reminder
                ...
                if task.dateOrTime == 0 {
                    ...
                    //If not time was set -> wants reminder when task starts
                    if reminderTime == [0,0] {
                        reminderButton.setTitle("When Task Starts", for: .normal)
                    } else {
                        reminderButton.setTitle("\(reminderTime[0]) \(hourString), \(reminderTime[1]) min before", for: .normal)
                    }
                } else {
...
func setupReminderTime() {
    ...
    if TaskService.shared.getDateOrTime() == 0 {
        ...
        let dateFormat = DateFormatter()
        dateFormat.dateFormat = "E MMM d, h:mm a"
        let date = dateFormat.string(from: TaskService.shared.getReminderDate())
        reminderButton.setTitle("\(date)", for: .normal)
    }
